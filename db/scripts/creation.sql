...
-- ------------- TABLE COMMENTAIRE TRAITE PAR ---------------
-- Création de la table
--
-- Rajouter une table pour faire le lien entre les salariés et les commentaires qu'ils ont traités
CREATE TABLE IF NOT EXISTS COMMENTAIRE_TRAITE_PAR(
   ID_COMMENTAIRE		INT NOT NULL,
   ID_SALARIE        INT NOT NULL,
   FOREIGN KEY (ID_COMMENTAIRE)
   REFERENCES COMMENTAIRES(IDENTIFIANT)
   ON DELETE CASCADE,
   FOREIGN KEY (ID_SALARIE)
   REFERENCES PERSONNEL(IDENTIFIANT)
   ON DELETE CASCADE,
   PRIMARY KEY (ID_COMMENTAIRE, ID_SALARIE)
);