...
function alimenteCommentaires($number, $nom, $prenom, $courriel, $note, $avis, $est_valide, $admin = false) {
    ...
    $commentaire .= '<div class="formulaire_deux_colonnes">
            <textarea id="Message" rows="10" , cols="40">'.$avis.'</textarea>
        </div>';
    if ($admin) {    
        $commentaire .= '<div class="feedback formulaire_contact">
                <div class="formulaire_label">
                    <label for="nom">Est validée : </label>
                </div>
                <div class="formulaire_champ">
                <input type="checkbox" id="est_valide" name="est_valide" '.($est_valide? 'checked' : '').'>
                </div>
            </div>';
    }
