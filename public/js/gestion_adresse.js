...
const formAdresse = document.getElementById("adresse_form");
...
formAdresse.addEventListener("submit", (event) => {
  ...
  if (formAdresse["adresse_rue1"].value === "") {
  ...
  if (donneesManqantes) {
    ...
    message += "</ul></p>";
    document.getElementById("formulaire_erreurs_adresse").innerHTML = message;
  } else {
    const formData = new FormData(formAdresse); // Create a FormData object with the form data
